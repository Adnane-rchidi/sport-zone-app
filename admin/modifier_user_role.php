...
if ($user_id_to_edit == $_SESSION['user_id']) {
    header("Location: admin_users.php?error=" . urlencode("You cannot change your own role."));
    exit;
}

$stmt = $conn->prepare("SELECT id, name, email, role FROM utilisateurs WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!in_array($new_role, $allowed_roles)) {
        ...
        if ($stmt->execute()) {
            header("Location: admin_users.php?msg=" . urlencode("User role updated successfully for " . $user_to_edit['name'] . "."));
            exit;
        } else {
            $errors[] = "Error updating user role: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Change User Role - Admin - SportZone</title>
...
<body>
    ...
    <div class="container main-content">
        <div class="content-container">
            <h2 class="mb-4">Change Role for <?= htmlspecialchars($user_to_edit['name']) ?></h2>
            <p class="text-center text-muted mb-4">Current email: <?= htmlspecialchars($user_to_edit['email']) ?></p>
            ...
            <form method="POST" action="">
                <div class="mb-3">
                    ...
                    <select name="role" id="role" class="form-select" required>
                        <?php foreach ($allowed_roles as $role_option): ?>
                            <option value="<?= $role_option ?>" <?= ($user_to_edit['role'] === $role_option) ? 'selected' : '' ?>>
                                <?= ucfirst($role_option) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="admin_users.php" class="btn btn-secondary back-link">
