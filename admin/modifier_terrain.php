...
$stmt = $conn->prepare("SELECT * FROM terrains WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$terrain = $stmt->get_result()->fetch_assoc();
$stmt->close();

if (!$terrain) {
    header("Location: admin_terrains.php?error=" . urlencode("Field not found."));
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($name) || empty($address) || empty($sport_id)) {
        ...
        $stmt = $conn->prepare("UPDATE terrains SET name=?, addresse=?, sports_id=?, is_active=? WHERE id=?");
        $stmt->bind_param("ssiii", $name, $address, $sport_id, $is_active, $id);
        if ($stmt->execute()) {
            header("Location: admin_terrains.php?msg=" . urlencode("Field updated successfully."));
            exit;
        } else {
            $errors[] = "Error updating field: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Edit Field - Admin - SportZone</title>
...
<body>
    ...
    <div class="container main-content">
        <div class="content-container">
            <h2 class="mb-4">Edit Field: <?= htmlspecialchars($terrain["name"]) ?></h2>
            <p class="text-center">
            ...
            <form method="POST" action="">
                <div class="mb-3">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="<?= htmlspecialchars($terrain["name"]) ?>" required>
                </div>

                <div class="mb-3">
                    <label for="addresse" class="form-label">Address:</label>
                    <textarea id="addresse" name="addresse" rows="3" class="form-control" required><?= htmlspecialchars($terrain["addresse"]) ?></textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="sport_id" id="sport_id" class="form-select" required>
                        <?php while ($s = $sports->fetch_assoc()): ?>
                            <option value="<?= $s["id"] ?>" <?= $s["id"] == $terrain["sports_id"] ? "selected" : "" ?>>
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" id="is_active" name="is_active" value="1" class="form-check-input" <?= $terrain["is_active"] ? 'checked' : '' ?>>
